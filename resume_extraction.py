...
if uploaded_file is not None: 
    ...
    duplicate_files = [x for i, x in enumerate(list_all_files) if list_all_files.count(x) > 1]
    set_duplicates = set(duplicate_files)
    st.write(set_duplicates)
    ...
    #st.write(len(list_unique_filenames))
    
    #for loop for extracting data for multiple files 
    for item_unique_filename in list_unique_filenames:
        ...
        for item in uploaded_file:
            ...
            if item_unique_filename == fileName and insert_flag == False:
                ...
                if phone_number:
                    #removing hyphen from dates 12-38-59
                    for num in phone_number:
                        if re.search(r'\-', num):#checks for presence of hyphen
                            #print('if',num)
                            phone_number.remove(num)
                        elif re.search(r'\.', num):#checks for presence of dot
                            #print('elif',num)
                            phone_number.remove(num)
                        elif re.search(r'\s+', num): #checks for presence of space
                            #print('elif',num)
                            phone_number.remove(num)
                        else:
                            #print('else',num)
    ...
    if final_table.empty == False:
        st.write(final_table.style.format({"Work Experience": "{:.2f}"}))
        
        #download for table data in csv format
        csv = convert_df(final_table)    
        st.download_button("Press to Download", csv, "resume_data.csv", "text/csv", key='download-csv') 
